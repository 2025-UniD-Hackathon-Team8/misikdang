...
function renderAction(
  ...
  return (
    <Button
      ...
      className={`h-[40px] ${
        action.fullWidth ? "w-full" : "w-[155px]"
      } text-sm ${isSecondary ? "border border-[var(--color-gray-2)]" : ""} ${
...
export default function RequestCardLarge({
  title = "",
  subtitle = "",
      ...
      {(leftAction || rightAction) && (
        <div
          className={`flex w-full ${
            leftAction?.fullWidth || rightAction?.fullWidth
              ? "pt-2"
